...
なのでこれから先Rustのエラーハンドリングがどうなっていくのかは注視する必要があるでしょう。
:::
...
ただしこれはあまりにもミスするのが簡単なため、もっと簡単にコンテクストをプリントできるようにしようとする[動き](https://blog.rust-lang.org/inside-rust/2021/07/01/What-the-error-handling-project-group-is-working-towards.html)があるようです。
...
### デメリット
デメリットは詳細な値の取得が難しいことです。`downcast_ref`という関数で詳細の値を得ることは可能ですが、煩雑になるのでこの関数を多用するようなら`Enum`でエラーを構成するほうがよいでしょう。


# まとめ Result<T, E>のEはなににするのがよいか
エラーの値が単一なら好きにしたらよいでしょう。ただ`Error`トレイトは実装した方がよいと思います。(Box化されたエラーに変換できるので)
複数の種類のエラーが返される場合は`Enum`(`thiserror`)を使うか`Box`(`anyhow`, `eyre`)を使うかです。
エラーの詳細の値を扱いたい場合は`Enum`、ただエラーを報告したいだけのときは`Box`を使うというのが定説みたいです。
アプリケーションのコードではただエラーを報告したいということが多いので`Box`(`anyhow`, `eyre`)を使うのが安定なことが多いでしょう。
